...
def _pv_linux_machine(machine):
    ...
    hardware_info_temp1 = [x for x in cpu_info.split('\n') if 'Hardware' in x]
    
    # Hacky modification:
    if len(hardware_info_temp1) == 0:
      return 'j-nano-arm64-v8'
    else:
      hardware_info = hardware_info_temp1[0]
...
def _library_path():
    if _PV_SYSTEM == 'Darwin':
        ...
        elif _PV_MACHINE == 'j-nano-arm64-v8':
            return os.path.join(os.path.dirname(__file__), '../../../lib/android/arm64-v8a/libpv_porcupine.so')
