...
import com.vva.vvamobileclient.ControlApp;
import com.vva.vvamobileclient.Core.Plans.RobotPlan;
import com.vva.vvamobileclient.R;
...
import vva_rosjava_service.VVARecognizeSpeech;
import vva_rosjava_service.VVARecognizeSpeechRequest;
import vva_rosjava_service.VVARecognizeSpeechResponse;
...
public class RobotController implements NodeMain, Savable {
    ...
    private static final String PAUSED_PLAN_BUNDLE_ID = "com.vva.vvamobileclient.Core.RobotController.pausedPlan";
...
//        rotation = pose.getOrientation();
    ...
    /**
     * Calls the ROS service for Speech Recognition
     */
    public void rc_requestSpeechRecognition(String wav_filename) {

        ServiceClient<VVARecognizeSpeechRequest, VVARecognizeSpeechResponse> serviceClient;
        try {
            serviceClient = connectedNode.newServiceClient("/vva_recognize_speech", VVARecognizeSpeech._TYPE);
        } catch (ServiceNotFoundException e) {
            throw new RosRuntimeException(e);
        }

        final VVARecognizeSpeechRequest request = serviceClient.newMessage();
        // Set the mic_type_id or client_id: The Speech Recognition ROS node will give special treatment to
        // the submitted audio data depending on this value. Possible values: "kinect", "mobile_app".
        request.setClientId("mobile_app");

        //Log.d(TAG, "Writing .wav file data in the ROS service request audio_buffer...");

        try {
            byte[] my_byte_array = FileUtils.readFileToByteArray(new File(wav_filename));
            ChannelBuffer my_audio_buffer = ChannelBuffers.copiedBuffer(ChannelBuffers.LITTLE_ENDIAN, my_byte_array);
            request.setAudioData(my_audio_buffer);
        } catch (IOException e) {
            throw new RosRuntimeException(e);
        }

        Log.i(TAG, "Calling ROS Service...");

        serviceClient.call(request, new ServiceResponseListener<VVARecognizeSpeechResponse>() {
            @Override
            public void onSuccess(VVARecognizeSpeechResponse response) {
                connectedNode.getLog().info(response.getStatus());
                //Log.d(TAG, "Service response status: " + response.getStatus());
            }
